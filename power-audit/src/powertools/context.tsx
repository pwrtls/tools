...
export class PowerToolsContextProvider extends React.PureComponent<IPowerToolsContextProviderProps, IPowerToolsContextProviderState> {
    ...
    componentDidMount() {
        ...
        console.log('Checking window.PowerTools:', typeof window.PowerTools);
        console.log('Window properties:', Object.keys(window).filter(k => k.includes('Power')));
        
        if (window.parent && window.parent !== window) {
            console.log('Running in iframe, parent window available');
            try {
                console.log('Can access parent properties:', Object.keys(window.parent).filter(k => k.includes('Power')));
            } catch (e) {
                console.log('Cannot access parent window properties (likely due to cross-origin restrictions):', e);
            }
        }
