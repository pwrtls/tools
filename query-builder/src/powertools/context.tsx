...
export class PowerToolsContextProvider extends React.PureComponent<IPowerToolsContextProviderProps, IPowerToolsContextProviderState> {
    ...
    componentDidMount() {
        if (typeof window.PowerTools === 'undefined') {
            console.error('PowerTools API not found');
        ...
        window.PowerTools.onLoad()
            ...
            .catch(() => this.setState({ invalidContext: true }));
    ...
    get invalidContextResult() {
        return (
            <div style={{ position: 'fixed', left: 0, top: 0, zIndex: 100, width: '100vw', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Result status="500" title="Invalid Context" subTitle="This tool was loaded with an invalid context, the PowerTools API is not within the context of this page." />
            </div>
        );
    }
    ...
    get noConnectionResult() {
        return (
            <div style={{ position: 'fixed', left: 0, top: 0, zIndex: 100, width: '100vw', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Result status="404" title="No Connection Selected" subTitle="To continue using this tool, please select a connection! Thank you" />
            </div>
        );
    }
